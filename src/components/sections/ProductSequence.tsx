    ...
    >
      ...
      >
        <div className="seq-copy-wrap mx-auto max-w-(--container-content) px-4 md:px-6">
          ...
          {/* §5.6 — NO scroll reveal on these rows, deliberately.
              They carry the product's five selling points. Three separate
              attempts to make a reveal reliable here (per-row observers, a
              visibility-checked failsafe, one observer with a CSS stagger) all
              passed automated scroll traces and all still left rows blank on a
              real phone. A mechanism that can hide main content and cannot be
              verified on the device that breaks it is not worth an entrance
              animation. DESIGN.md §5 already says motion that does not cover a
              visitor-caused state change should be deleted; this is that rule
              applied after the fact. Do not reintroduce one. */}
          <ul className="seq-copy-list flex flex-col divide-y divide-hairline md:gap-4 md:divide-y-0">
            {FEATURE_KEYS.map((key, i) => {
              ...
              return (
                ...
                >
                  {/* Plain div: keeps the DOM depth `Enter` produced so the
