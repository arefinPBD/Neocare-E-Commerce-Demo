  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden="true"
        /* group-data-[at-top]/header comes from Header's named group: --nc-ink-500 fails WCAG
           1.4.11 over the hero photograph. See the note in Header.tsx. */
        className="h-6 w-6 flex-shrink-0 text-ink-500 transition-colors duration-[--dur-fast] group-hover:text-fg-muted group-data-[at-top=true]/header:text-fg"
      >
      ...
      {hydrated ? (
        <span
          aria-live="polite"
          className="ml-2 min-w-3 type-small font-semibold text-fg-muted transition-colors duration-[--dur-fast] group-hover:text-fg group-data-[at-top=true]/header:text-fg"
        >
          {itemCount}
        </span>
      ) : (
