        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-surface-alt">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="text-fg-muted">
          ...
          <>
            <div className="flex-1 overflow-y-auto px-4 md:px-6">
              <div className="divide-y divide-hairline">
                {items.map((item) => (
                  <CartItemRow key={`${item.sizeKey}:${item.pack}`} item={item} t={t} locale={locale} />
                ))}
              </div>
            </div>
